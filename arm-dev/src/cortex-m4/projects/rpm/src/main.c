...
{	
	...
	while(1)
	{
		...
		TIM_SetCompare3(TIM3, ADC_GetConversionValue(ADC1));
		TIM_SetCompare4(TIM3, ADC_GetConversionValue(ADC1));
...
void configTIM3(void)
{
	...
	//PB0 and PB1 are set to generate PWM output channels 3 and 4
...
void configTIM4(void)
{
	...
	uint16_t UEV = 10;	//UEV frequency (Hz)
